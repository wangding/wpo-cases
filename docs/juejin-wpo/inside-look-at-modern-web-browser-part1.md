...
![4 个 CPU 核心作为办公人员，坐在办公桌前处理各自的工作](./img/inside-browser-part1-1.png)
...
![许多带特定扳手的 GPU 内核意味着它们只能处理有限任务](./img/inside-browser-part1-2.png)
...
![三层计算机体系结构。底部是机器硬件，中间是操作系统，顶部是应用程序](./img/inside-browser-part1-3.png)

## 在进程和线程上执行程序

![进程作为边界框，线程作为抽象鱼在进程中游动](./img/inside-browser-part1-4.png)
...
![进程使用内存空间和存储应用数据的示意图](./img/inside-browser-part1-5-memory.svg)
...
![独立进程通过 IPC 通信示意图](./img/inside-browser-part1-6-workerprocess.svg)

## 浏览器架构

那么如何通过进程和线程构建 web 浏览器呢？它可能由一个拥有很多线程的进程，或是一些通过 IPC 通信的不同线程的进程。

![不同浏览器架构的进程/线程示意图](./img/inside-browser-part1-7.png)
...
![Chrome 的多进程架构示意图。渲染进程下显示了多个层，表明 Chrome 为每个标签页运行多个渲染进程。](./img/inside-browser-part1-8.png)
...
![不同进程指向浏览器 UI 的不同部分](./img/inside-browser-part1-9.png)
...
![如图所示每个标签页上运行的渲染进程](./img/inside-browser-part1-10-tabs.svg)
...
![Chrome 的服务化图，将不同的服务移动到多个进程和单个浏览器进程中](./img/inside-browser-part1-11-servicfication.svg)
...
![站点隔离示意图，多个渲染进程指向站点内的 iframe](./img/inside-browser-part1-12.png)
