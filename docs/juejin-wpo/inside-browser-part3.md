...
![渲染进程内部包含主线程、工作线程、合成线程和光栅线程](./img/inside-browser-part3-1.png)
...
![主线程解析 HTML 并构建 DOM 树](./img/inside-browser-part3-2.png)
...
![主线程解析 CSS 以添加计算后样式](./img/inside-browser-part3-3.png)
...
![一个人站在一幅画前，电话线与另一个人相连](./img/inside-browser-part3-4.png)
...
![主线程遍历计算样式后的 DOM 树，以此生成布局树](./img/inside-browser-part3-5.png)

![由于换行而移动的盒子布局](./img/inside-browser-part3-6.mp4)
...
![一个人拿着笔站在画布前，思考着她应该先画圆形还是先画方形](./img/inside-browser-part3-7.png)

拥有 DOM、样式和布局仍然不足以渲染页面。假设你正在尝试重现一幅画。你知道元素的大小、形状和位置，但你仍需要判断绘制它们的顺序。

例如，可以为某些元素设置 `z-index`，此时按 HTML 中编写的元素的顺序绘制会导致错误的渲染。

![因为没有考虑 z-index，页面元素按 HTML 标记的顺序出现，导致错误的渲染图像](./img/inside-browser-part3-8.png)

在绘制步骤中，主线程遍历布局树创建绘制记录。绘制记录是绘图过程的记录，就像是“背景优先，然后是文本，然后是矩形”。如果你使用过 JavaScript 绘制了 `<canvas>` 元素，那么这个过程对你来说可能很熟悉。

![主线程遍历布局树并生成绘制记录](./img/inside-browser-part3-9.png)

### 更新渲染管道的成本很高

![DOM + Style、布局和绘制树的生成顺序](./img/inside-browser-part3-10.gif)
...
![时间轴上的动画帧](./img/inside-browser-part3-11.png)

即使渲染操作能跟上屏幕刷新，这些计算也会在主线程上运行，这意味着当你的应用程序运行 JavaScript 时动画可能会被阻塞。

![时间轴上的动画帧，但 JavaScript 阻塞了一帧](./img/inside-browser-part3-12.png)
...
![时间轴上较小的 JavaScript 块与动画帧一起运行](./img/inside-browser-part3-13.png)

## 合成

### 如何绘制一个页面？

![简单光栅处理示意动画](./img/inside-browser-part3-14.gif)
...
![合成处理示意动画](./img/inside-browser-part3-15.gif)
...
![主线程遍历布局树生成图层树](./img/inside-browser-part3-16.png)
...
![光栅线程创建分块的位图并发送到 GPU](./img/inside-browser-part3-17.png)
...
![合成线程创建合成帧，将其发送到浏览器进程，再接着发送到 GPU](./img/inside-browser-part3-18.png)
